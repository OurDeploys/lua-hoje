...
import { Moon, MoonStar, Clock, Share2, ChevronUp, Calendar, Info } from "lucide-react"
...
export default function Home() {
  ...
  useEffect(() => {
    const calcularFaseLua = () => {
      ...
      const iluminacao = Math.round(50 * (1 - Math.cos(2 * Math.PI * faseDecimal)))
      ...
      const proximaData = new Date(agora.getTime() + diasProximaFase * 24 * 60 * 60 * 1000)
  ...
  const getFaseDescription = (fase: string) => {
    const descricoes: { [key: string]: string } = {
      "Lua Nova": "A lua não é visível no céu. É o momento ideal para novos começos e definir intenções.",
      "Lua Crescente": "A lua está crescendo em luminosidade. Tempo de crescimento e desenvolvimento de projetos.",
      "Lua Cheia": "A lua está completamente iluminada. Momento de culminação, celebração e gratidão.",
      "Lua Minguante Gibosa": "A lua está diminuindo após a lua cheia. Tempo de reflexão e liberação.",
      "Lua Minguante": "A lua está em fase decrescente. Momento de soltar o que não serve mais.",
    }
    ...
    <main className="flex min-h-screen flex-col items-center bg-black text-white overflow-x-hidden">
      <div className="container max-w-md px-4 py-8 flex flex-col items-center">
          ...
          <CardContent className="p-6">
            <div className="flex flex-col items-center justify-center gap-6">
              ...
              <div className="text-center">
                ...
                <p className="text-xl text-zinc-400">{luaInfo.iluminacao}% de iluminação</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-center w-full mb-8">
          <Button variant="ghost" size="lg" className="flex items-center gap-2" onClick={handleCompartilhar}>
        ...
        <div className="text-center text-zinc-500 text-sm mb-4">
          ...
          >
            ...
            <span>Deslize para cima para mais detalhes</span>
          </Button>
        </div>
        ...
        {mostrarDetalhes && (
          <div ref={detalhesRef} className="w-full space-y-6 animate-in slide-in-from-bottom duration-500">
            <Card className="w-full bg-zinc-900 border-zinc-800">
              ...
              <CardContent>
                <p className="text-zinc-300 leading-relaxed">{getFaseDescription(luaInfo.fase)}</p>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">{luaInfo.proximaFase}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-zinc-400">Data:</span>
                    <span className="font-semibold">{luaInfo.proximaData}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-zinc-400">Em:</span>
                    <span className="font-semibold">
                      {luaInfo.diasProximaFase} {luaInfo.diasProximaFase === 1 ? "dia" : "dias"}
                    </span>
                  </div>
                </div>
              </CardContent>
